...
题解: 建树，如果能够成功建成，那么就是BST或者MIBST，否则就不是二叉搜索树。建树的过程判断是否符合搜索树或镜像搜索树的条件。
...
tree *createTree(tree *t, int key) {
    if(!t) {
        t = new tree;
        t->val = key;
        t->lchild = NULL;
        ...
        if(key < t->val) {
            t->lchild = createTree(t->lchild , key);
        } else {
            t->rchild = createTree(t->rchild, key);
        }
    }
    return t;
}
